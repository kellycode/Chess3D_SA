...
(function () {
    ...
    function playMove(piece, cell) {
        ...
        if (
            (p & 0x7) === piecePawn &&
            ((start.y === 1 && g_playerWhite) || (start.y === 6 && !g_playerWhite)) &&
            ((p & 0x8 && g_playerWhite) || (!(p & 0x8) && !g_playerWhite))
        ) {
            testPromotion = true;
        }
        ...
        for (var i = 0; i < validMoves.length; i++) {
            if (testPromotion) {
                ...
                // castling
                if ((validMoves[i] & 0xff) == startSquare && ((validMoves[i] >> 8) & 0xff) == endSquare) {
                    move = validMoves[i];
                    break;
                }
            }
        ...
        for (var i in board3D) {
            if ({}.hasOwnProperty.call(board3D, i)) {
                ...
                if (intersect.length > 0) {
                    hit = intersect[0];
                    if (
                        (g_playerWhite && hit.object.parent.color === WHITE) ||
    ...
    function onDocumentMouseMove(event) {
        ...
        if (selectedPiece !== null && pickedCell !== null) {
            ...
            for (var i = 0; i < validMoves.length; i++) {
                if (
                    (validMoves[i] & 0xff) == MakeSquare(start.y, start.x) &&
